 ...
 * Vinky Sites Notices
 *
 * Closing notice on click on `vinky-notice-close` class.
 *
 ...
 * `<a href="#" data-repeat-notice-after="<?php echo MONTH_IN_SECONDS; ?>" class="vinky-notice-close">`
...
if ( ! class_exists( 'Vinky_Notices' ) ) :
	...
	class Vinky_Notices {
		...
		public function __construct() {
			...
			add_action( 'wp_ajax_vinky-notice-dismiss', array( $this, 'dismiss_notice' ) );
		 ...
		 *
		 * @param array $allowed_post_tags array of allowed tags.
		 *
		 * @return array
		 * @since 1.0.0
		 */
		public function add_data_attributes( $allowed_post_tags ) {
			$allowed_post_tags['a']['data-repeat-notice-after'] = true;
		...
		public function dismiss_notice() {
			...
			if ( ! apply_filters( 'vinky_notices_user_cap_check', current_user_can( 'manage_options' ) ) ) {
				return;
			}
			...
			if ( false === wp_verify_nonce( $nonce, 'vinky-notices' ) ) {
				wp_send_json_error( esc_html__( 'WordPress Nonce not validated.', 'vinky' ) );
			}
		...
		public function enqueue_scripts() {
			wp_register_script( 'vinky-notices', self::_get_uri() . 'notices.js', array( 'jquery' ), self::$version, true );
			wp_localize_script(
				'vinky-notices',
				'vinkyNotices',
				array(
					'_notice_nonce' => wp_create_nonce( 'vinky-notices' ),
				)
			);
		}
		...
		public function show_notices() {
			...
			$defaults = array(
				'id'                         => '',      // Optional, Notice ID. If empty it set `vinky-notices-id-<$array-index>`.
				...
				'display-with-other-notices' => true,    // Should the notice be displayed if other notices  are being displayed from Vinky_Notices.
				'is_dismissible'             => true,
			);
		...
		public static function markup( $notice = array() ) {
			...
			wp_enqueue_script( 'vinky-notices' );

			do_action( 'vinky_notice_before_markup' );

			do_action( "vinky_notice_before_markup_{$notice['id']}" );
			...
			<div id="<?php echo esc_attr( $notice['id'] ); ?>" class="<?php echo esc_attr( $notice['classes'] ); ?>" data-repeat-notice-after="<?php echo esc_attr( $notice['repeat-notice-after'] ); ?>">
				<div class="notice-container">
					<?php do_action( "vinky_notice_inside_markup_{$notice['id']}" ); ?>
					<?php echo wp_kses_post( $notice['message'] ); ?>
				</div>
			</div>
			<?php

			do_action( "vinky_notice_after_markup_{$notice['id']}" );

			do_action( 'vinky_notice_after_markup' );
		...
		private static function get_wrap_classes( $notice ) {
			$classes = array( 'vinky-notice', 'notice' );
