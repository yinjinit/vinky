	...
	class Vinky_Admin_Settings {
		...
		public static function init_admin_settings() {
			...
			if ( strpos( $requested_page, self::$plugin_slug ) !== false ) {
				...
				if ( defined( 'DIGIWAKE_EXT_VER' ) && version_compare( DIGIWAKE_EXT_VER, '2.5.0', '<' ) ) {
					self::save_settings();
				}
			}
		...
		public static function register_notices() {
			...
			if ( current_user_can( 'install_plugins' ) && ! defined( 'DIGIWAKE_SITES_NAME' ) && '1' === get_option( 'fresh_site' ) ) {

				wp_enqueue_script( 'vinky-admin-settings' );
				$image_path           = get_theme_file_uri( 'inc/assets/images/vinky-logo.svg' );
				$dgw_sites_notice_btn = array();

				$dgw_sites_notice_btn['button_text'] = __( 'Import Layout', 'vinky' );
				$dgw_sites_notice_btn['link']        = 'edit.php?post_type=dgw_templates';
				$dgw_sites_notice_btn['class']      .= ' button button-primary';
				...
				$vinky_sites_notice_args = array(
					...
					'message'                    => sprintf(
						...
						__( 'Vinky provides dozens of <a href="https://www.vinky.com/pre-made">pre-made layout</a> to build your website faster. Import your favorite site or a specific page from the layout packs.', 'vinky' ),
						esc_attr( $dgw_sites_notice_btn['class'] ),
						'href="' . $dgw_sites_notice_btn['link'] . '"',
						esc_html( $dgw_sites_notice_btn['button_text'] )
					),
					'priority'                   => 5,
		...
		public static function add_admin_menu() {
			...
			if ( apply_filters( 'vinky_dashboard_admin_menu', true ) ) {
				...
				add_theme_page( $builder_title, $builder_title, $capability, 'edit.php?page=dgw_theme_builder' );
				add_theme_page( $layout_title, $layout_title, $capability, 'edit.php?post_type=dgw_templates' );
			} else {
		...
		public static function menu_callback() {
			...
			$dgw_wrapper_class = apply_filters( 'vinky_welcome_wrapper_class', array( $current_slug ) );

			?>
			<div
				class="dgw-menu-page-wrapper wrap dgw-clear <?php echo esc_attr( implode( ' ', $dgw_wrapper_class ) ); ?>">
		...
		public static function vinky_welcome_page_knowledge_base_section() {
			...
			<div class="postbox">
				<h2 class="hndle dgw-normal-cursor">
				...
				<div class="inside">
					...
					<?php
					$vinky_knowledge_base_doc_link      = apply_filters( 'vinky_knowledge_base_documentation_link', vinky_get_pro_url( 'https://www.vinky.com/docs/', 'vinky-dashboard', 'visit-documentation', 'welcome-page' ) );
		...
		public static function vinky_welcome_page_community_section() {
			...
			<div class="postbox">
				...
				<div class="inside">
					...
					<?php
					$vinky_community_group_link      = apply_filters( 'vinky_community_group_link', 'https://community.vinky.com' );
		...
		public static function vinky_welcome_page_five_star_section() {
			...
			<div class="postbox">
				...
				<div class="inside">
					...
					<?php
					$vinky_support_link      = apply_filters( 'vinky_support_link', vinky_get_pro_url( 'https://www.vinky.com/contact/', 'vinky-dashboard', 'submit-a-ticket', 'welcome-page' ) );
		...
		public static function vinky_welcome_page_content() {
			...
			<div class="postbox">
				...
				<div class="dgw-quick-setting-section">
					<?php
					if ( ! empty( $quick_settings ) ) :
						?>
						<div class="dgw-quick-links">
							<ul class="dgw-flex">
								<?php
								foreach ( (array) $quick_settings as $key => $link ) {
									echo '<li class=""><span class="dashicons ' . esc_attr( $link['dashicon'] ) . '"></span><a class="dgw-quick-setting-title" href="' . esc_url( $link['quick_url'] ) . '" target="_blank" rel="noopener">' . esc_html( $link['title'] ) . '</a></li>';
								}
								?>
							</ul>
						</div>
		...
		public static function vinky_available_plugins() {
			...
			if ( apply_filters( 'vinky_show_free_extend_plugins', true ) ) {
				...
				<div class="postbox">
					<h2 class="hndle dgw-normal-cursor dgw-addon-heading dgw-flex">
						<span><?php echo esc_html( $vinky_addon_tagline ); ?></span>
					</h2>
					<div class="dgw-addon-list-section">
						<?php
						if ( ! empty( $recommended_plugins ) ) :
							?>
							<div>
								<ul class="dgw-addon-list">
									<?php
									foreach ( $recommended_plugins as $slug => $plugin ) {
										...
										echo '<div class="dgw-addon-link-wrapper">';
